...
class AppDrawer extends StatelessWidget {
  ...
  Widget _buildDrawerListTile(
    ...
    return Material(
      ...
      child: ListTile(
        leading:
            Icon(icon, color: selected ? selectedIconColor : defaultIconColor),
  ...
  Widget build(BuildContext context) {
    ...
    const double maxAbsoluteWidth =
        304.0; // Drawer의 최대 절대 너비 (Material Design 가이드라인 참고)
    ...
    final double baseBodyMediumFontSize =
        theme.textTheme.bodyMedium?.fontSize ?? 14.0;
    final double mainDrawerItemFontSize =
        baseBodyMediumFontSize * 0.9; // 10% smaller than bodyMedium
    final double subDrawerItemFontSize =
        baseBodyMediumFontSize * 0.8; // 20% smaller than bodyMedium
    ...
    final TextStyle selectedMainDrawerItemTextStyle = TextStyle(
        ...
        fontSize: mainDrawerItemFontSize);
    final Color defaultDrawerIconColor = theme.iconTheme.color ??
        theme.colorScheme.onSurface.withOpacity(0.7); // This variable is used.
          ...
          children: <Widget>[
                ...
                children: [
                  ...
                  if (EventManager.isEventPeriodActive()) // 이벤트 기간에만 표시
                    _buildDrawerListTile(
                      ...
                      listItemTextStyle: mainDrawerItemTextStyle,
                      selectedListItemTextStyle:
                          selectedMainDrawerItemTextStyle,
                      defaultIconColor: defaultDrawerIconColor,
                      selectedIconColor: selectedDrawerIconColor,
                    ),
                  Theme(
                    // ExpansionTile의 기본 구분선 색상을 투명하게 만듦
                    data: theme.copyWith(dividerColor: Colors.transparent),
                    child: ExpansionTile(
                      key: const PageStorageKey<String>(
                          'simulator_expansion_tile'), // 상태 유지를 위한 키
                      leading: Icon(Icons.precision_manufacturing_rounded,
                          color: defaultDrawerIconColor),
                      title: showDrawerText
                          ? Text('시뮬레이터', style: mainDrawerItemTextStyle)
                          : const SizedBox.shrink(),
                      tilePadding: const EdgeInsets.symmetric(
                          horizontal: 16.0, vertical: 0.0), // ListTile과 유사한 패딩
                      iconColor:
                          theme.colorScheme.secondary, // 확장 아이콘 색상 (열렸을 때)
                      collapsedIconColor:
                          defaultDrawerIconColor, // 확장 아이콘 색상 (닫혔을 때)
                      childrenPadding: EdgeInsets.only(
                          left: showDrawerText ? 16.0 : 0), // 하위 항목 들여쓰기
                      children: <Widget>[
                        ListTile(
                          dense: true,
                  ...
                  _buildDrawerListTile(
                    ...
                    onTap: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text('백과사전 기능은 아직 준비 중입니다.')));
                    },
                    showDrawerText: showDrawerText,
                    theme: theme, // Pass theme
                    listItemTextStyle: mainDrawerItemTextStyle,
            ...
            Padding(
              ...
              child: _buildDrawerListTile(
                ...
                onTap: () {
                  Navigator.of(context).pop();
                  if (currentScreen != AppScreen.appSettings) {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
