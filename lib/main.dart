...
class MyApp extends StatelessWidget {
  ...
  ThemeData _buildTheme(Brightness brightness, double fontSizeMultiplier) {
    ...
    final double deviceResponsiveFontScale =
        1.0 * newBaseMultiplier; // 화면 너비 반응형 로직을 일단 상수로 대체
...
// 파란색
    ...
    // 토글 색상 정의 (초록색 계열)
    const Color toggleGreenLight =
        Color(0xFF388E3C); // Material Design Green 700
...
// 어두운 파랑 (Blue 800)
    ...
    const Color toggleGreenDark =
        Color(0xFF00E676); // Material Design Green Accent 400

    // 밝기 및 색상 설정 (런타임 값인 isDark에 의존하므로 const가 될 수 없음)
    final Color primaryColor = isDark
        ? const Color.fromARGB(255, 30, 3, 124)
        : const Color.fromARGB(255, 0, 121, 202);
    final Color accentColor = isDark ? accentColorDark : accentColorLight;
    final Color scaffoldBackgroundColor =
        isDark ? scaffoldBackgroundColorDark : scaffoldBackgroundColorLight;
    final Color cardBackgroundColor =
        isDark ? cardBackgroundColorDark : cardBackgroundColorLight;
    ...
    final Color onPrimaryColor =
        isDark ? Colors.white : Colors.white; // 기본 색상 위에 흰색 텍스트/아이콘
    final Color onSecondaryColor = isDark
        ? Colors.white
        : Colors.black87; // 강조 색상 위에 텍스트/아이콘 (밝은 모드는 어둡게, 다크 모드는 밝게)
    final Color errorColor =
        isDark ? const Color(0xFFC62828) : Colors.red; // 오류 색상 (다크 모드는 Red 800)
    const Color onErrorColor = Colors.white; // 오류 색상 위에 흰색 텍스트/아이콘
    final Color onSurfaceColor = isDark
        ? Colors.white
        : const Color(0xFF06202B); // 밝은 배경 색상 위에 어두운 텍스트/아이콘
    final Color inputFillColor = isDark
        ? cardBackgroundColorDark
        : Colors.grey[100]!; // cardBackgroundColorDark는 라인 34의 것을 사용
    final Color dividerColor =
        isDark ? Colors.grey[700]! : const Color.fromARGB(255, 223, 223, 223);
    // line 43 부근의 hintColor는 isDark에 의존하므로 const가 될 수 없습니다.
    final Color hintColor = isDark
        ? secondaryTextColorDark.withAlpha((255 * 0.7).round())
        : secondaryTextColorLight.withAlpha(150);

    // 기본 TextTheme 가져오기 (런타임 값인 isDark 및 ThemeData 메서드 호출에 의존하므로 const가 될 수 없음)
    final TextTheme originalTextTheme =
        isDark ? ThemeData.dark().textTheme : ThemeData.light().textTheme;

    // 글꼴 크기 배율 적용하여 TextTheme 생성 (originalTextTheme 및 fontSizeMultiplier에 의존하므로 const가 될 수 없음)
    final TextTheme customTextTheme = originalTextTheme
        .copyWith(
          bodyLarge: originalTextTheme.bodyLarge?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.bodyLarge?.fontSize ?? 14.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          bodyMedium: originalTextTheme.bodyMedium?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.bodyMedium?.fontSize ?? 14.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          bodySmall: originalTextTheme.bodySmall?.copyWith(
              color: secondaryTextColor,
              fontSize: (originalTextTheme.bodySmall?.fontSize ?? 12.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          displayLarge: originalTextTheme.displayLarge?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.displayLarge?.fontSize ?? 96.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          displayMedium: originalTextTheme.displayMedium?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.displayMedium?.fontSize ?? 60.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          displaySmall: originalTextTheme.displaySmall?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.displaySmall?.fontSize ?? 48.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          headlineLarge: originalTextTheme.headlineLarge?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.headlineLarge?.fontSize ?? 34.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          headlineMedium: originalTextTheme.headlineMedium?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.headlineMedium?.fontSize ?? 24.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          headlineSmall: originalTextTheme.headlineSmall?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.headlineSmall?.fontSize ?? 20.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier), // AppBar title 기본
          titleLarge: originalTextTheme.titleLarge?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.titleLarge?.fontSize ?? 20.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier), // Dialog title, List Title 등
          titleMedium: originalTextTheme.titleMedium?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.titleMedium?.fontSize ?? 16.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier), // ListTile title, Input label 등
          titleSmall: originalTextTheme.titleSmall?.copyWith(
              color: secondaryTextColor,
              fontSize: (originalTextTheme.titleSmall?.fontSize ?? 14.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          labelLarge: originalTextTheme.labelLarge?.copyWith(
              color: onSecondaryColor,
              fontSize: (originalTextTheme.labelLarge?.fontSize ?? 14.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier), // Button text
          labelMedium: originalTextTheme.labelMedium?.copyWith(
              color: textColor,
              fontSize: (originalTextTheme.labelMedium?.fontSize ?? 12.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
          labelSmall: originalTextTheme.labelSmall?.copyWith(
              color: secondaryTextColor,
              fontSize: (originalTextTheme.labelSmall?.fontSize ?? 10.0) *
                  deviceResponsiveFontScale *
                  fontSizeMultiplier),
        )
        .apply(
    ...
    return ThemeData(
      ...
      colorScheme: ColorScheme(
        ...
        surface:
            cardBackgroundColor, // Card 배경 등에 사용 (isDark에 따라 light/dark 배경 사용)
        onSurface: onSurfaceColor,
        surfaceContainerHighest:
            cardBackgroundColor, // Dialog 배경 등에 사용 (Material 3)
      ...
      appBarTheme: AppBarTheme(
        ...
        elevation: 4.0,
        titleTextStyle:
            customTextTheme.headlineSmall?.copyWith(color: onPrimaryColor),
      ...
      elevatedButtonTheme: ElevatedButtonThemeData(
        style: ElevatedButton.styleFrom(
          ...
          textStyle:
              customTextTheme.labelLarge?.copyWith(fontWeight: FontWeight.bold),
        ),
      ),
      ...
      inputDecorationTheme: InputDecorationTheme(
        ...
        border: OutlineInputBorder(
          ...
          borderSide:
              isDark ? BorderSide.none : BorderSide(color: Colors.grey[300]!),
        ),
        ...
        labelStyle:
            customTextTheme.titleMedium?.copyWith(color: secondaryTextColor),
      ...
      dialogTheme: DialogThemeData(
        backgroundColor: isDark
            ? cardBackgroundColorDark
            : cardBackgroundColorLight, // 다크 모드에서 검정 배경 사용
      ...
      sliderTheme: SliderThemeData(
        ...
        valueIndicatorTextStyle:
            customTextTheme.labelSmall?.copyWith(color: onPrimaryColor),
      ),
...
// 다크 모드 OFF 상태 썸 기본값
        ...
        trackOutlineColor:
            WidgetStateProperty.resolveWith<Color?>(// Material 3 스위치 외곽선 색상
                (Set<WidgetState> states) {
          if (!isDark) {
            // 일반 모드
            if (states.contains(WidgetState.selected)) {
              // ON 상태
              return Colors
                  .transparent; // 외곽선 없음 또는 toggleGreenLight와 동일하게 설정 가능
            } else {
              // OFF 상태
      ...
      drawerTheme: DrawerThemeData(
        backgroundColor:
            isDark ? cardBackgroundColorDark : scaffoldBackgroundColorLight,
      ),
        ...
        return ValueListenableBuilder<double>(
          ...
          builder: (_, double currentFontSizeMultiplier, __) {
            return MaterialApp(
              ...
              darkTheme:
                  _buildTheme(Brightness.dark, currentFontSizeMultiplier),
