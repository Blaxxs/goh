...
class ExplorationOptionSimulationScreenUI extends StatelessWidget {
  ...
  Widget _buildAutoChangeCard(
      BuildContext context, ThemeData theme, bool isDark) {
    final grades = ExplorationOptionGrade.values;
    final currentIndex = grades.indexOf(autoTargetGrade);
    final prevGrade =
        grades[(currentIndex - 1 + grades.length) % grades.length];
    final nextGrade = grades[(currentIndex + 1) % grades.length];
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            IconButton(
              onPressed: () => onSetAutoTargetGrade(prevGrade),
              icon: const Icon(Icons.chevron_left),
              visualDensity: VisualDensity.compact,
            ),
            Container(
            ...
            IconButton(
              onPressed: () => onSetAutoTargetGrade(nextGrade),
              icon: const Icon(Icons.chevron_right),
              visualDensity: VisualDensity.compact,
            ),
            const SizedBox(width: 8),
            ElevatedButton(
              onPressed: isAutoRunning ? onStopAutoChange : onStartAutoChange,
