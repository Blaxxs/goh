...
class AccessoryEnhancementScreenUI extends StatelessWidget {
  ...
  static const Map<int, Map<String, double>> baseEnhancementProbabilities = {
    // static으로 유지하되, private 해제하여 외부 참조 가능하도록
  ...
  // 강화 보조제별 성공 확률 보너스
  static const Map<String, double> enhancementAidBonuses = {
    // static으로 유지하되, private 해제
    '선택 안함': 0.0, // 기본값
    '하급 보조제': 0.02, // 성공확률 +2%
    '중급 보조제': 0.05, // 성공확률 +5%
    '상급 보조제': 0.10, // 성공확률 +10%
  ...
  static final List<Map<String, String>> _aidImageDetails = [
    ...
    {
      'name': '스페셜 하급 보조제',
      'path': '$_baseAidImagePath/special_low_grade_aid.png'
    },
    {
      'name': '스페셜 중급 보조제',
      'path': '$_baseAidImagePath/special_mid_grade_aid.png'
    },
    {
      'name': '스페셜 상급 보조제',
      'path': '$_baseAidImagePath/special_high_grade_aid.png'
    },
    {
      'name': '스페셜 특급 보조제',
      'path': '$_baseAidImagePath/special_super_grade_aid.png'
    },
  ];
  ...
  Widget build(BuildContext context) {
    ...
    // 강화 버튼 활성화 조건 수정
    final bool canManuallyEnhance =
        !isAutoEnhancing && isAccessorySelected && currentEnhancementLevel < 9;
    final bool canStartAutoEnhance = isAutoEnhanceMode &&
        !isAutoEnhancing &&
        targetEnhancementLevel != null &&
        currentEnhancementLevel < targetEnhancementLevel! &&
        currentEnhancementLevel < 9;
    final bool canPressEnhanceButton = !isAutoEnhancing &&
        onEnhanceButtonPressed != null &&
        isAccessorySelected && // 악세사리가 선택되어야 하고
        currentEnhancementLevel < 9 && // 현재 레벨이 9 미만이어야 하고
        (isAutoEnhanceMode
            ? canStartAutoEnhance
            : canManuallyEnhance); // 자동 모드 조건 또는 수동 모드 조건 만족
    ...
    final double aidBonusValue =
        enhancementAidBonuses[selectedEnhancementAid] ?? 0.0;
    bool isSpecialAidNoDowngrade = selectedEnhancementAid.startsWith('스페셜') &&
        selectedEnhancementAid != '스페셜 특급 보조제';
    ...
    final String failNoChangeText =
        "${(finalFailNoChangeChance * 100).toStringAsFixed(0)}%";
    final String downgradeText =
    ...
    final int stoneCost =
        (currentEnhancementLevel < 9 && costs != null) ? costs['stones']! : 0;
    final int goldCost =
    ...
    return Scaffold(
      ...
      body: !isAccessorySelected
          ? Center(
              ...
              // 악세사리 미선택 시 선택 영역 - 이 GestureDetector는 유지합니다.
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      crossAxisAlignment: CrossAxisAlignment
                          .baseline, // 세로 정렬을 baseline으로 변경하여 텍스트 기준 정렬
                        ...
                        Flexible(
                          ...
                          child: Text(
                            ...
                            style: theme.textTheme
                                .titleMedium, // 폰트 사이즈 더 줄임 (titleLarge -> titleMedium)
                            textAlign: TextAlign.center,
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: currentEnhancementLevel > 0
                              ? () => onCurrentEnhancementLevelChanged(
                        ...
                        ...List.generate(9, (index) {
                          return Icon(
                            index < currentEnhancementLevel
                                ? Icons.star_rounded
                                : Icons.star_border_rounded,
                            color: index < currentEnhancementLevel
                                ? Colors.amber
                                : Colors.grey[400],
                            size: 30,
                          );
                    ...
                    if (isAccessorySelected) // 악세사리가 선택된 경우에만 표시
                      SwitchListTile(
                        ...
                        title: Text(
                          ...
                          style: theme.textTheme.titleMedium?.copyWith(
                              color:
                                  isAutoEnhancing ? theme.disabledColor : null),
                        ),
                        value: isAutoEnhanceMode,
                        onChanged: isAutoEnhancing
                            ? null
                            : onAutoEnhanceModeChanged, // 강화 중에는 스위치 비활성화
                        secondary: Icon(isAutoEnhanceMode
                            ? Icons.autorenew_rounded
                            : Icons.play_arrow_rounded),
                        contentPadding: const EdgeInsets.symmetric(
                            horizontal: 0), // 내부 패딩 조절
                      ),
                    if (isAccessorySelected)
                      const SizedBox(height: 8), // 자동 강화 스위치 아래 간격 조정 (16 -> 8)

                    // 목표 강화 단계 (자동 강화 모드일 때만 표시) 및 강화 보조제 선택
                    Row(
                      // 이 Row는 isAccessorySelected가 true일 때만 표시됨
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      crossAxisAlignment: CrossAxisAlignment
                          .start, // DropdownButton 높이 다를 수 있으므로 상단 정렬
                      children: [
                        ...
                        if (isAccessorySelected)
                          Expanded(
                            child: Column(
                              crossAxisAlignment:
                                  CrossAxisAlignment.center, // 문구와 아이콘을 중앙 정렬
                              children: [
                                Text('강화 보조제',
                                ...
                                GestureDetector(
                                  onTap: () =>
                                      _showEnhancementAidSelectionDialog(
                                          context,
                                          theme,
                                          onEnhancementAidChanged),
                                  child: _buildCurrentAidDisplay(
                                      theme), // Display + icon or large selected aid image
                                ),
                              ],
                            ), // 강화 보조제 Column 끝
                          ),
                        ...
                        if (isAccessorySelected) // 악세사리가 선택된 경우에만 두 번째 자식 영역을 그림
                          Expanded(
                            child: isAutoEnhanceMode
                                ...
                                : const SizedBox
                                    .shrink(), // 자동 강화 OFF: 빈 공간 (Expanded가 빈 공간을 차지하지 않도록)
                          ),
                      ],
                    ),
                    ...
                    isAutoEnhancing
                        ...
                        : ElevatedButton.icon(
                            // 평상시 "강화" 또는 "자동 강화 시작" 버튼
                            icon: Icon(isAutoEnhanceMode
                                ? Icons.autorenew_rounded
                                : Icons.upgrade_rounded),
                            label: Text(isAutoEnhanceMode ? '자동 강화 시작' : '강화',
                                style: const TextStyle(fontSize: 18)),
                            style: ElevatedButton.styleFrom(
                              minimumSize: const Size(double.infinity, 50),
                            ),
                            onPressed: canPressEnhanceButton
                                ? onEnhanceButtonPressed
                                : null,
                          ),
      ...
      if (imagePath != null) {
        return Image.asset(
          ...
          errorBuilder: (ctx, err, st) => const Icon(Icons.broken_image,
              size: 80), // Fallback for large image
        );
      }
  ...
  static String? _getAidImagePath(String aidName) {
    final aidData = _aidImageDetails.firstWhere((aid) => aid['name'] == aidName,
        orElse: () => {'path': ''});
    return aidData['path']!.isNotEmpty ? aidData['path'] : null;
  }

  // 보조제별 기대 비용 비교 테이블
  Widget _buildAidComparisonTable(BuildContext context, ThemeData theme,
      int baseStones, NumberFormat fmt) {
    final colorScheme = theme.colorScheme;
    final level = currentEnhancementLevel;
    final baseProbs = baseEnhancementProbabilities[level] ??
        {'success': 0.0, 'fail_no_change': 0.0, 'downgrade': 0.0};

    // 각 보조제별 기대 시도 횟수 계산 (단순 모델: +1 레벨 달성 기댓값)
    List<_AidCompareRow> rows = [];
    for (final entry in enhancementAidBonuses.entries) {
      final aidName = entry.key;
      final bonus = entry.value;

      double finalSuccess;
      if (aidName == '스페셜 특급 보조제') {
        finalSuccess = 1.0;
      } else {
        final baseFailNoChange = baseProbs['fail_no_change']!;
        final bonusApply = bonus > baseFailNoChange ? baseFailNoChange : bonus;
        finalSuccess = baseProbs['success']! + bonusApply;
      }

      if (finalSuccess <= 0) continue;
      final double expectedTries = 1.0 / finalSuccess;
      final int expectedStones = (expectedTries * baseStones).round();

      rows.add(_AidCompareRow(
        name: aidName,
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              '보조제별 기대 비용 비교 (${level}→${level + 1}단계)',
              style: theme.textTheme.titleSmall
                  ?.copyWith(fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 8),
            Table(
              ...
              children: [
                ...
                for (final row in rows)
                  TableRow(
                    decoration: row.expectedStones == minStones
                        ? BoxDecoration(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Text(row.name,
                            style: theme.textTheme.bodySmall?.copyWith(
                              fontWeight: row.expectedStones == minStones
                                  ? FontWeight.bold
                                  : null,
                            )),
                      ),
                      Padding(
                        padding: const EdgeInsets.symmetric(vertical: 3),
                        child: Text(
                          '${(row.successRate * 100).toStringAsFixed(0)}%',
                          textAlign: TextAlign.center,
                          style: theme.textTheme.bodySmall?.copyWith(
                            color: Colors.green[700],
                            fontWeight: row.expectedStones == minStones
                      ...
                      Padding(
                        padding: const EdgeInsets.symmetric(vertical: 3),
                        child: Text(
                          fmt.format(row.expectedStones),
                          textAlign: TextAlign.right,
                          style: theme.textTheme.bodySmall?.copyWith(
                            color: row.expectedStones == minStones
                                ? colorScheme.primary
                                : null,
                            fontWeight: row.expectedStones == minStones
                                ? FontWeight.bold
            ...
            Text(
              '※ 기대 영혼석 = 1회 소모 × 평균 시도 횟수 (다운그레이드 미반영)',
  ...
  void _showEnhancementAidSelectionDialog(BuildContext context, ThemeData theme,
      ValueChanged<String?> onAidSelected) {
    Widget buildImageRow(List<Map<String, String>> imagesSublist) {
      return Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: imagesSublist.map((imgMap) {
          return Expanded(
            // 이미지가 공간을 균등하게 차지하도록 Expanded 추가
            child: InkWell(
              onTap: () {
                // 변경: 전달받은 onAidSelected 콜백 사용
    ...
    for (int i = 0; i < _aidImageDetails.length; i += itemsPerRow) {
      int end = (i + itemsPerRow < _aidImageDetails.length)
          ? i + itemsPerRow
          : _aidImageDetails.length;
  ...
  void _showConsumedAidsDialog(
      BuildContext context, ThemeData theme, Map<String, int> consumedCounts) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
