...
class AccessoryOptionChangeScreenUI extends StatelessWidget {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Section 2: Options
            if (isAccessorySelected) ...[
              // Only show if accessory is selected
              Text('현재 옵션',
            ...
            if (isAccessorySelected) ...[
              // This block remains, only the button inside is removed
              Text('변경 비용',
                  style: theme.textTheme.titleLarge?.copyWith(
                      fontWeight: FontWeight.bold)), // Moved title outside Card
              const SizedBox(height: 8), // Spacing between title and card
              _buildCostInfo(
                  context, theme), // This now only builds the cost content
              const SizedBox(height: 24),
            ]
          ],
        ),
          ...
          children: [
            Text(
              ...
              style: theme.textTheme
                  .titleMedium, // Reduced font size for accessory name
              textAlign: TextAlign.center,
            ),
  ...
  Widget _buildCostInfo(BuildContext context, ThemeData theme) {
    ...
    if (totalSoulStonesConsumed > 0) {
      costWidgets.add(Text(
          '영혼석: ${_formatter.format(totalSoulStonesConsumed)}개',
          style: theme.textTheme.bodyLarge,
          textAlign: TextAlign.center));
    }
    if (totalGrindstonesConsumed > 0) {
      costWidgets.add(Text(
          '숫돌이: ${_formatter.format(totalGrindstonesConsumed)}개',
          style: theme.textTheme.bodyLarge,
          textAlign: TextAlign.center));
    }
    if (totalRainbowAnvilsConsumed > 0) {
      costWidgets.add(Text(
          '무지개 모루: ${_formatter.format(totalRainbowAnvilsConsumed)}개',
          style: theme.textTheme.bodyLarge,
          textAlign: TextAlign.center));
    }
    if (total9EnhanceAccessoriesConsumed > 0) {
      costWidgets.add(Text(
          '3옵 9강 악세: ${_formatter.format(total9EnhanceAccessoriesConsumed)}개',
          style: theme.textTheme.bodyLarge,
          textAlign: TextAlign.center));
    }

    if (costWidgets.isEmpty) {
      costWidgets.add(Text('아직 소모된 재화가 없습니다.',
  ...
  Widget _buildOptionRow(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(
              width: 40, child: Text(slot, style: theme.textTheme.titleMedium)),
          Expanded(
            child: InkWell(
              // Wrap with InkWell for tap functionality
              onTap: onTapOptionArea,
              borderRadius:
                  BorderRadius.circular(4.0), // Optional: for visual feedback
              child: Padding(
                padding: const EdgeInsets.symmetric(
                    vertical: 4.0, horizontal: 8.0), // Add padding for tap area
                child: Row(
                  // Use a Row to separate name and value, now with dynamic spacing
                  mainAxisAlignment:
          ...
          if (actionButton != null)
            actionButton, // Only show if actionButton is provided
        ],
      ),
    );
  }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (i < currentOptionCount) {
        ...
        if (isBaseOptionSlot) {
          ...
          if (i == 2) {
            // 3rd option (expanded from 2-op accessory)
            onTapOptionArea =
                () => widget.onActionSelected(OptionChangeAction.changeThird);
          } else if (i == 3) {
            // 4th option
            onTapOptionArea =
                () => widget.onActionSelected(OptionChangeAction.changeFourth);
          }
        }
      } else {
        // This slot is not a base option slot and is currently empty (meaning it's expandable)
        optionName = '비어있음'; // Default for empty slots
        optionValue = null;
        optionTextColor = theme.hintColor;

        if (i == 2) {
          // Expanding to 3rd option
          if (widget.selectedAccessory.options.length < 3) {
            onTapOptionArea =
                () => widget.onActionSelected(OptionChangeAction.expandToThird);
          }
        } else if (i == 3) {
          // Expanding to 4th option
          // Can only expand to 4th if 3rd option exists (currentOptions.length >= 3)
          if (currentOptionCount >= 3) {
            onTapOptionArea = () =>
                widget.onActionSelected(OptionChangeAction.expandToFourth);
          }
        }
      }
