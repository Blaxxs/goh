...
class _SpiritScreenState extends State<SpiritScreen> {
  ...
  Set<String> _expandSearchToken(String normalizedToken) {
    ...
    if (normalizedToken.contains('크리티컬') && normalizedToken.contains('저항')) {
      expanded.addAll({
        '크저',
        '크리티컬저항',
      });
    ...
    for (final entry in _searchSynonyms.entries) {
      ...
      final isMatched =
          canonical == normalizedToken || synonyms.contains(normalizedToken);
      if (isMatched) {
  ...
  Set<String> _searchAliasesForTarget(String target) {
    ...
    if (normalized.contains('크리티컬') && normalized.contains('저항')) {
      aliases.addAll({
        '크저',
        '크리티컬저항',
      });
