...
class _JournalScreenState extends State<JournalScreen> {
  ...
  void initState() {
    ...
    _startingDayOfWeek =
        SettingsService.instance.appSettings.startingDayOfWeek; // 초기값 로드
  ...
  double _calculateFinalGoldForEntry(JournalEntry entry) {
    ...
    if (stageDataForEntry != null) {
      final List<dynamic>? dropsDynamic =
          stageDataForEntry['drops'] as List<dynamic>?;
      if (dropsDynamic != null) {
        final List<DropInfo> drops =
            dropsDynamic.whereType<DropInfo>().toList();
        final int goldDemonDropIndex = drops.indexWhere((d) =>
            d.category == DropCategory.goldDemon &&
            goldDemonSellPrices.containsKey(d.itemId));
        final DropInfo? goldDemonDrop =
            (goldDemonDropIndex != -1) ? drops[goldDemonDropIndex] : null;
  ...
  Future<void> _updateMonthlyTotalsAndShares() async {
    ...
    final DateTime monthToProcess =
        DateTime(_focusedDay.year, _focusedDay.month);
    List<JournalEntry> allEntries =
        SettingsService.instance.getAllJournalEntries();
    List<JournalEntry> monthEntries = allEntries.where((entry) {
      return entry.date.year == monthToProcess.year &&
          entry.date.month == monthToProcess.month;
    }).toList();
    ...
    for (var entry in monthEntries) {
      ...
      stageSoulStoneSums[entry.stage] =
          (stageSoulStoneSums[entry.stage] ?? 0) + entry.netSoulStones;
      stageGoldSums[entry.stage] =
          (stageGoldSums[entry.stage] ?? 0) + entryFinalGold;
    }
  ...
  Future<void> _showCalendarSettingsDialog() async {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: StartingDayOfWeek.values
                .where((day) =>
                    day == StartingDayOfWeek.sunday ||
                    day == StartingDayOfWeek.monday)
                .map((day) {
              // 일요일과 월요일만 제공
    ...
    if (selected != null && selected != _startingDayOfWeek) {
      await SettingsService.instance
          .updateAppSettings(startingDayOfWeek: selected);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: const AppDrawer(
          currentScreen: AppScreen.journal), // 현재 화면을 journal로 설정
          ...
          children: [
            ...
            Padding(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _currentShareType == JournalShareType.soulStone
                      ? '$_goldLabel 점유율 보기'
                      : '$_soulStoneLabel 점유율 보기',
                ),
                onPressed: () {
                  if (mounted) {
                    setState(() {
            ...
            TableCalendar(
              ...
              availableGestures:
                  AvailableGestures.horizontalSwipe, // 수평 스와이프만 허용
              ...
              headerStyle: const HeaderStyle(
                ...
                titleTextStyle: TextStyle(
                    fontSize: 18.0, fontWeight: FontWeight.w500), // 헤더 제목 스타일
              ...
              calendarStyle: CalendarStyle(
                ...
                defaultTextStyle: TextStyle(
                    ...
                    color: theme.textTheme.bodyLarge
                        ?.color), // Added missing comma here
                holidayTextStyle: const TextStyle(
                    color: Colors.blue,
                    fontSize: 15.0,
                    fontWeight: FontWeight.w500), // 토요일 스타일
                holidayDecoration:
                    const BoxDecoration(shape: BoxShape.rectangle), // 기본 모양 유지
              ),
              holidayPredicate: (day) {
                // 토요일을 휴일로 간주하여 스타일 적용
                return day.weekday == DateTime.saturday;
              },
              calendarBuilders: CalendarBuilders(
                dowBuilder: (context, day) {
                  // 요일 표시 커스텀 빌더를 calendarBuilders 내부로 이동
                ...
                markerBuilder: (context, date, events) {
                  final entriesForDay = SettingsService.instance
                      .getEntriesForDay(date); // SettingsService에서 일지 목록 가져옴
                  if (entriesForDay.isNotEmpty) {
                    ...
                    if (dailyNetSoulStones.abs() < 0.001) {
                      // 값이 0이면 마커 표시 안 함
                      return null;
                    }

                    final formatter = NumberFormat(
                        '+ #,##0; - #,##0; 0', 'ko_KR'); // 양수, 음수, 0 포맷
                    String displayText;
                    Color textColor;
  ...
  Widget _buildJournalEntryList() {
    ...
    debugPrint(
        '[_buildJournalEntryList] Building list for selected day: $_selectedDay');

    final entries = SettingsService.instance.getEntriesForDay(
        _selectedDay ?? DateTime.now()); // SettingsService에서 일지 목록 가져옴
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final stageData = stageBaseData[
            entry.stage]; // _calculateFinalGoldForEntry와 중복 로직, 추후 정리 가능
        if (stageData != null) {
          final List<dynamic>? dropsDynamic =
              stageData['drops'] as List<dynamic>?;
          if (dropsDynamic != null) {
            final List<DropInfo> drops =
                dropsDynamic.whereType<DropInfo>().toList();
            final int goldDemonDropIndex = drops.indexWhere((d) =>
            ...
            if (goldDemonDrop != null) {
              final int demonPrice =
                  goldDemonSellPrices[goldDemonDrop.itemId] ?? 0;
              finalGold += (entry.earnedGoldDemons * demonPrice);
            }
          }
        }
                ...
                onPressed: (context) {
                  ...
                  if (_selectedDay != null) {
                    SettingsService.instance
                        .removeJournalEntry(_selectedDay!, entry); // 일지 삭제
                    _updateMonthlyTotalsAndShares().then((_) {
                      // 월별 데이터 재계산 후 UI 갱신
                      if (mounted) setState(() {});
                    });
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: theme.textTheme.bodySmall?.copyWith(
                        fontSize:
                            (theme.textTheme.bodySmall?.fontSize ?? 12) * 0.95),
                  ),
  ...
  Widget _buildMonthlyShareDisplay() {
    ...
    final percentFormatter =
        NumberFormat('0\'%\'', 'ko_KR'); // 소수점 없이 정수만 표시, % 기호 포함
    ...
    if (_currentShareType == JournalShareType.soulStone) {
      ...
      titleLine1 =
          '${_focusedDay.month}월 총 획득 $_soulStoneLabel: ${formatter.format(_totalMonthSoulStones.round())}';
    } else {
      currentMonthSums = _monthlyStageGoldSums;
      currentMonthPercentages = _monthlyStageGoldSharesPercent;
      titleLine1 =
          '${_focusedDay.month}월 총 획득 $_goldLabel: ${formatter.format(_totalMonthGold.round())}';
    }

    if (currentMonthSums.isEmpty) {
      return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
        child: Text('이번 달 기록된 데이터가 없거나 점유율을 계산할 수 없습니다.',
    ...
    String valueLine = sortedStageEntries
        .map((entry) =>
            '${entry.key}: ${formatter.format(currentMonthSums[entry.key]?.round() ?? 0)}')
        .join(', ');

    // 셋째 줄: 백분율 표시
    String percentageLine = sortedStageEntries
        .map((entry) =>
            '${entry.key}: ${percentFormatter.format(entry.value.floor())}') // entry.value.floor()로 소수점 버림
        .join(', ');
