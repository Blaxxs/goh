...
class _AddJournalDialogState extends State<AddJournalDialog> {
  ...
  void _saveJournalEntry() {
    ...
    final int? consumedSoulStones =
        int.tryParse(_consumedSoulStonesController.text);
    ...
    final int? earnedSoulStones =
        int.tryParse(_earnedSoulStonesController.text);
    ...
    final int earnedGoldDemons =
        int.tryParse(_goldDemonCountController.text) ?? 0;
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        // 얼럿의 가로 길이를 30% 키우기 위해 SizedBox로 감쌈
        width: MediaQuery.of(context).size.width *
            0.91, // 화면 너비의 약 91% (기존 70%에서 30% 증가)
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              DropdownButtonFormField2<String>(
                decoration: InputDecoration(
                  ...
                  contentPadding: const EdgeInsets.symmetric(
                      vertical: 14.4,
                      horizontal:
                          12.0), // contentPadding은 decoration 안에 있어야 합니다.
                ),
                isExpanded:
                    true, // isExpanded는 DropdownButtonFormField2의 직접적인 매개변수입니다.
                hint: Center(
                  // hint도 DropdownButtonFormField2의 직접적인 매개변수입니다.
                  child: Text(
                    '선택',
                ...
                items: stageNames.map((String stage) {
                  // items는 DropdownButtonFormField2의 직접적인 매개변수입니다.
                  return DropdownMenuItem<String>(
                    value: stage,
                    child: Center(
                        child: Text(stage,
                            overflow: TextOverflow.ellipsis)), // 아이템 텍스트 중앙 정렬
                  );
                }).toList(),
                ...
                buttonStyleData: const ButtonStyleData(
                    height: 23,
                    padding: EdgeInsets.only(right: 8)), // 버튼 높이를 23으로 되돌림
                    ...
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelStyle:
                          ...
                          fontSize: (theme.inputDecorationTheme.labelStyle
                                      ?.fontSize ??
                                  16.0) *
                              0.8, // 폰트 크기 20% 감소
                        ),
                        ...
                        contentPadding: const EdgeInsets.symmetric(
                            horizontal: 12.0, vertical: 12.0), // 내부 여백
                      ),
                    ),
                  ),
              ...
              Row(
                crossAxisAlignment:
                    CrossAxisAlignment.start, // 필드 높이가 다를 경우 상단 정렬
                children: [
                  Expanded(
                    // 획득 골드 입력 필드
                    flex: 2,
